...
def get_di_date(d):
    """
    Pulls the DIxPRE data from table available at the URL
    """
...
file_path = os.getcwd() + '\\outDIxPRE.xlsx'
df_d.to_excel(file_path, engine = 'openpyxl')
wd_adapt = wd.replace('/','-')
server_dest_path = get_download_path() + r'\DIxPRE - Dia {0}.xlsx'.format(wd_adapt)
